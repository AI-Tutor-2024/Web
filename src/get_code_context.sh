...
directories=("components" "pages" "app" "(route)" "api" "styles" "utils" "hooks" "constants" "services" "types" "layout.tsx" "page.tsx" )
  ...
  do
    if [ -d "$entry" ]; then
      ...
      if ! $should_ignore; then
        ...
        echo "Processing file: $relative_path" # 디버깅 로그 추가
...
for dir in "${directories[@]}"; do
  if [ -d "${project_dir}/${dir}" ]; then
    echo "Reading directory: ${project_dir}/${dir}" # 디버깅 로그 추가
    read_files "${project_dir}/${dir}"
  else
    echo "Directory does not exist: ${project_dir}/${dir}" # 디버깅 로그 추가
  fi
done
